...
router.route("/category").post(admin,createJobCategory).get(admin,getAllJobCategocry);
router.route("/category/:id").patch(admin,updateJobCategory)
router.route("/:id").delete(admin,deleteJob).patch(upload.single('file'),admin,updateJob).get(admin,getIndividualjob);
